...
SUBGRAPH = SubGraph(
    name="GI_PLATFORM_BOARD_B",
    entry="STEP_COMPUTE_BOUNDS_B",
    exits={"success": "BOARD_DONE_B"}
)

@step
def STEP_COMPUTE_BOUNDS_B(worker, cycle, env):
    ...
    return Next("STEP_BOARD_COORDS_B")

@step
def STEP_BOARD_COORDS_B(worker, cycle, env):
    ...
    return Next("STEP_TPL_PAINT_B")

@step
def STEP_TPL_PAINT_B(worker, cycle, env):
    ...
    out = env.transform("template_map", items=squares,
                        ...
                        round_coords=True)
    cycle.setdefault("mc", {})["paint_cmds_b"] = out.get("commands") or []
    return Next("STEP_DO_PAINT_B")

@step
def STEP_DO_PAINT_B(worker, cycle, env):
    cmds = cycle.get("mc", {}).get("paint_cmds_b", [])
    env.tool("minecraft_control", operation="batch_commands", delay_ms=2, commands=cmds)
    return Next("STEP_MSG_BOARD_DONE_B")

@step
def STEP_MSG_BOARD_DONE_B(worker, cycle, env):
    env.tool("minecraft_control", operation="execute_command", command="say [INIT] Board painted (B)")
    return Exit("success")
