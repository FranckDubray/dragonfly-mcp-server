...
SUBGRAPH = SubGraph(
    name="GI_CLEAR_AREA_B",
    entry="STEP_COMPUTE_AABB_B",
    exits={"success": "CLR_DONE_B"}
)

@step
def STEP_COMPUTE_AABB_B(worker, cycle, env):
    ...
    return Next("STEP_CLEAR_AIR_B")

@step
def STEP_CLEAR_AIR_B(worker, cycle, env):
    p = cycle.get("plat", {})
    env.tool("minecraft_control", operation="execute_command",
             command=f"fill {p['x1']} {p['y_min']} {p['z1']} {p['x2']} {p['y_max']} {p['z2']} minecraft:air")
    return Next("STEP_KILL_AABB_B")

@step
def STEP_KILL_AABB_B(worker, cycle, env):
    p = cycle.get("plat", {})
    dx = p['x2'] - p['x1']
    dy = p['y_max'] - p['y_min']
    dz = p['z2'] - p['z1']
    ...
    return Next("STEP_MSG_CLEAR_DONE_B")

@step
def STEP_MSG_CLEAR_DONE_B(worker, cycle, env):
    env.tool("minecraft_control", operation="execute_command",
