...
from .parse_constit import parse_constit_member
from .validators import validate_canonical

LOGGER = logging.getLogger("legifrance_ingestion.bootstrap_constit_minimal")

DEFAULT_MAX_DECISIONS = 20


def compute_datasource_path(obj: dict) -> str:
    return f"constit/decision/{obj['id']}.json"


def run_minimal_constit_roundtrip(
    archive_path: str,
    ...
    for member in filter_xml_members(iter_archive_members(archive)):
        try:
            raw_objects = parse_constit_member(member)
        ...
        for raw_obj in raw_objects:
            ...
            published_count += 1
            LOGGER.info("Published CONSTIT decision | path=%s", path)

        if published_count >= max_decisions:
            break

    if published_count == 0:
        raise RuntimeError("No CONSTIT decision object could be parsed and published from archive")

    run_name = f"constit_bootstrap_minimal_{archive.stem.replace('.', '_')}"
