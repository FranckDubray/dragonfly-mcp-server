...
LOGGER = logging.getLogger("legifrance_ingestion.parse_constit")


@dataclass
class ParsedConstitObject:
    entity_type: str
    corpus: str
    id: str
    title: str
...
def _detect_kind(member_name: str, root_tag: str) -> str:
    ...
    if tag in {"TEXTE_JURI_CONSTIT", "DECISION", "CONSTIT"}:
        return "decision"

    if "/constit/" in name and name.endswith('.xml'):
        return "decision"

    raise ValueError(f"Unsupported CONSTIT kind for member={member_name} root_tag={root_tag}")


def parse_constit_decision(root: ET.Element) -> ParsedConstitObject:
    ...
    formation = _text_or_none(root, './/META/META_SPEC/META_JURI_CONSTIT/FORMATION')
    ...
    ecli = _text_or_none(root, './/META/META_SPEC/META_JURI_CONSTIT/ECLI')
...
def parse_constit_member(member: ArchiveMember) -> list[dict[str, Any]]:
    root = ET.fromstring(member.content)
    kind = _detect_kind(member.member_name, root.tag)

    if kind == 'decision':
        return [parse_constit_decision(root).to_raw_obj()]
