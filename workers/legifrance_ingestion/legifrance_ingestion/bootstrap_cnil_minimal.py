...
from .parse_cnil import parse_cnil_member
from .validators import validate_canonical

LOGGER = logging.getLogger("legifrance_ingestion.bootstrap_cnil_minimal")

DEFAULT_MAX_DECISIONS = 20


def compute_datasource_path(obj: dict) -> str:
    return f"cnil/decision/{obj['id']}.json"


def run_minimal_cnil_roundtrip(
    archive_path: str,
    ...
    for member in filter_xml_members(iter_archive_members(archive)):
        try:
            raw_objects = parse_cnil_member(member)
        ...
        for raw_obj in raw_objects:
            ...
            published_count += 1
            LOGGER.info("Published CNIL decision | path=%s", path)

        if published_count >= max_decisions:
            break

    if published_count == 0:
        raise RuntimeError("No CNIL decision object could be parsed and published from archive")

    run_name = f"cnil_bootstrap_minimal_{archive.stem.replace('.', '_')}"
