...
from .bootstrap_circulaires_minimal import run_minimal_circulaires_roundtrip
...
Corpus = Literal["legi", "jorf", "kali", "cass", "jade", "constit", "cnil", "circulaires"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--corpus",
        required=True,
        choices=["legi", "jorf", "kali", "cass", "jade", "constit", "cnil", "circulaires"],
...
def _resolve_bootstrap_archive(settings: Settings, corpus: Corpus, archive_path: str | None) -> str:
    ...
    if corpus == "cnil":
        ...
        return str(download_archive(settings, latest))
    if corpus == "circulaires":
        latest = select_latest_archive(list_circulaires_archives(settings))
        return str(download_archive(settings, latest))
...
def _run_single_archive(corpus: Corpus, archive_path: str) -> dict:
    ...
    if corpus == "circulaires":
        return run_minimal_circulaires_roundtrip(archive_path)
...
def _list_archives_for_corpus(settings: Settings, corpus: Corpus):
    ...
    if corpus == "circulaires":
        return list_circulaires_archives(settings)
...
def dispatch(settings: Settings, mode: Mode, corpus: Corpus, archive_path: str | None = None) -> int:
    ...
    if corpus == "circulaires" and not settings.enable_circulaires:
        raise RuntimeError("Corpus CIRCULAIRES is disabled by configuration")
