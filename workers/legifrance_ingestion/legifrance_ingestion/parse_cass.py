...
LOGGER = logging.getLogger("legifrance_ingestion.parse_cass")


@dataclass
class ParsedCassObject:
    entity_type: str
    corpus: str
    id: str
    title: str
...
def _detect_kind(member_name: str, root_tag: str) -> str:
    ...
    if tag in {"TEXTE_JURI_JUDI", "DECISION", "JURI"}:
        return "decision"

    if "/juri/" in name and "/cass/" in name and name.endswith('.xml'):
        return "decision"

    raise ValueError(f"Unsupported CASS kind for member={member_name} root_tag={root_tag}")


def parse_cass_decision(root: ET.Element) -> ParsedCassObject:
    ...
    formation = _text_or_none(root, './/META/META_SPEC/META_JURI_JUDI/FORMATION')
    ...
    ecli = _text_or_none(root, './/META/META_SPEC/META_JURI_JUDI/ECLI')
...
def parse_cass_member(member: ArchiveMember) -> list[dict[str, Any]]:
    root = ET.fromstring(member.content)
    kind = _detect_kind(member.member_name, root.tag)

    if kind == 'decision':
        return [parse_cass_decision(root).to_raw_obj()]
