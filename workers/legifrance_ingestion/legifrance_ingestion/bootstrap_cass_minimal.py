...
from .parse_cass import parse_cass_member
from .validators import validate_canonical

LOGGER = logging.getLogger("legifrance_ingestion.bootstrap_cass_minimal")

DEFAULT_MAX_DECISIONS = 20


def compute_datasource_path(obj: dict) -> str:
    return f"cass/{obj['entity_type']}/{obj['id']}.json"


def run_minimal_cass_roundtrip(
    archive_path: str,
    ...
    for member in filter_xml_members(iter_archive_members(archive)):
        try:
            raw_objects = parse_cass_member(member)
        ...
        for raw_obj in raw_objects:
            ...
            published_count += 1
            LOGGER.info("Published CASS decision | path=%s", path)

        if published_count >= max_decisions:
            break

    if published_count == 0:
        raise RuntimeError("No CASS decision object could be parsed and published from archive")

    run_name = f"cass_bootstrap_minimal_{archive.stem.replace('.', '_')}"
