...
from .parse_circulaires import parse_circulaires_member
from .validators import validate_canonical

LOGGER = logging.getLogger("legifrance_ingestion.bootstrap_circulaires_minimal")

DEFAULT_MAX_TEXTS = 20


def compute_datasource_path(obj: dict) -> str:
    return f"circulaires/{obj['entity_type']}/{obj['id']}.json"


def run_minimal_circulaires_roundtrip(
    archive_path: str,
    ...
    for member in filter_xml_members(iter_archive_members(archive)):
        try:
            raw_objects = parse_circulaires_member(member)
        except Exception as exc:
            LOGGER.warning(
                "Skipping member due to parse error | member=%s error=%s",
                member.member_name,
                exc,
            )
            continue
        ...
        for raw_obj in raw_objects:
            if raw_obj["entity_type"] != "texte":
                continue
            if published_count >= max_texts:
                break
            ...
            published_count += 1
            LOGGER.info("Published CIRCULAIRES text | path=%s", path)
    ...
    if published_count == 0:
        raise RuntimeError(
            "No CIRCULAIRES text object could be parsed and published from archive"
        )

    run_name = f"circulaires_bootstrap_minimal_{archive.stem.replace('.', '_')}"
    ...
    return {
        ...
        "mapping_paths": [],
        "published_counts": {"texte": published_count},
        "limits": {
            "max_texts": max_texts,
        },
    }
