...
def _spawn_runner(db_path: str) -> int:
    """Spawn detached runner process (OS-aware), return PID"""
...
def start(params: dict) -> dict:
    ...
    # Conflict check: if phase ∈ {running, sleeping} and heartbeat fresh → conflict
    ...
    # Load worker_file to compute process_uid and extract process_version
    try:
    ...
    # Persist state (including worker_name for runner to read)
    set_phase(db_path, worker_name, 'starting')
    set_state_kv(db_path, worker_name, 'cancel', 'false')
    set_state_kv(db_path, worker_name, 'worker_name', worker_name)  # NEW: persist worker_name
...
def stop(params: dict) -> dict:
    """Stop operation: set cancel flag or send signal"""
    p = validate_params(params)
    worker_name = p['worker_name']
    ...
    if not Path(db_path).exists():
        return {
            "accepted": False,
            "status": "error",
            "message": f"Worker '{worker_name}' not found (no DB)",
            "truncated": False
        }
    ...
    if mode == 'soft':
        ...
        return {
            "accepted": True,
            "status": "ok",
            "message": "Cancel flag set (cooperative shutdown)",
            "pid": pid,
            "db_path": db_path,
            "truncated": False
        }
    ...
    if not pid:
        return {
            "accepted": False,
            "status": "error",
            "message": "No PID found (cannot send signal)",
            "db_path": db_path,
            "truncated": False
        }
    
    # Signal-based stop (term/kill)
    try:
        if mode == 'term':
            if os.name == 'nt':
        ...
        return {
            "accepted": False,
            "status": "error",
            "message": f"Failed to send signal: {str(e)[:200]}",
            "pid": pid,
            "db_path": db_path,
            "truncated": False
        }
...
def status(params: dict) -> dict:
    """Status operation: read state from DB"""
    p = validate_params(params)
    worker_name = p['worker_name']
    ...
    if not Path(db_path).exists():
        return {
            ...
            "status": "unknown",
            "worker_name": worker_name,
            "message": "No DB found (worker never started or DB deleted)",
            "truncated": False
        }
    ...
    if process_version:
        result['process_version'] = process_version
    if last_error:
        result['last_error'] = last_error[:400]
    
    return result

def start_or_control(params: dict) -> Dict[str, Any]:
    """Main entry point: dispatch to start/stop/status"""
    ...
    try:
        ...
        return {
            "accepted": False,
            "status": "error",
            "message": f"Unexpected error: {str(e)[:350]}",
            "truncated": False
        }
