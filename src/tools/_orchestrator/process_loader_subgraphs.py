...
def flatten_subgraphs(process: Dict, base_dir: Path) -> Dict:
    ...
    for edge in main_edges:
        ...
        # IMPORTANT: once exit node is chosen based on 'when', the edge from that exit to the target
        # must be unconditional for the FSM (non-decision nodes emit route='always').
        # If we keep when='success' here, no route will match and the engine will drop the edge.
