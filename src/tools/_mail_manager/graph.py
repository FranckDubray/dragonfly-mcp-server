...
flowchart TD
  ...
  sleep_interval --> has_more_mailboxes

  imap_search_unseen_oldest --> has_unseen{has unseen ?}
  has_unseen -->|oui| imap_get_message_full(imap get message full)
  has_unseen -->|non| finish_mailbox_db(finish mailbox db)
  finish_mailbox_db --> has_more_mailboxes
  ...
  call_llm_classify_both --> class_branch{spam ham unsure ?}
  class_branch -->|spam| log_spam_db(log spam db)
  class_branch -->|ham| log_ham_db(log ham db)
  class_branch -->|unsure| log_unsure_db(log unsure db)

  log_spam_db --> imap_move_to_spam(imap move to spam)
  imap_move_to_spam --> has_unseen

  log_ham_db --> imap_mark_read(imap mark read)
  imap_mark_read --> has_unseen

  log_unsure_db --> has_unseen
"""
