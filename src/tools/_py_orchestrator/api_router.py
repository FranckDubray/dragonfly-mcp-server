...
def route(params: dict) -> Dict[str, Any]:
    try:
        ...
        return {"accepted": False, "status": "error", "code": "INVALID_OPERATION", "message": f"Invalid operation: {op}", "truncated": False}
    except Exception as e:
        # Best-effort: persist last_error if worker_name present
        try:
            ...
            if wn:
                ...
                set_state_kv(dbp, wn, 'last_error', str(e)[:400])
        except Exception:
            pass
        return {"accepted": False, "status": "error", "code": "ROUTE_ERROR", "message": str(e)[:400], "truncated": False}
