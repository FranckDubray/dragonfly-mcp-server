...
from ..runner_helpers import is_canceled
from .loader import load_worker_root, load_module
...
from .summary import persist_summary_if_any
from .preflight import preflight_load_graph, set_graph_metadata
from .reloader import maybe_hot_reload
...
def _load_process_and_subgraphs(root: Path, db_path: str, worker: str):
    ...
    return process, submods


def _arm_initial_pause_if_requested(db_path: str, worker: str, current_sub: str, current_step: str, cycle_id: str):
...
def run_loop(db_path: str, worker: str):
    ...
    process, submods = _load_process_and_subgraphs(root, db_path, worker)
    if process is None or submods is None:
        return
    ...
    _arm_initial_pause_if_requested(db_path, worker, current_sub, current_step, cycle_id)
    ...
    while not is_canceled(db_path, worker):
        ...
        # record previous_node -> executing transition
        try:
